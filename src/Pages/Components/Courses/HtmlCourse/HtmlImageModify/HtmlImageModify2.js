...
const HtmlImageModify2 = () => {
    ...
    const handleModify = event => {
        ...
        const updateImageData2 = {
            title: title,
        }

        // sending data to server
        const url = `http://localhost:5000/htmlImage/title/${id}`
        fetch(url, {
            method: 'PUT',
            headers: {
                'content-type': 'application/json'
            },
            body: JSON.stringify(updateImageData2)
        })
            .then(res => {
                console.log(res)
                return res.json()
...
export default HtmlImageModify2;